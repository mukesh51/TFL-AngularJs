(function(){
	var overgroundController = function($scope,$log,lineFactory){
		
  	$scope.selected = undefined;

  	$scope.showLiveDeparture = function(customSelected) {
       $log.log($scope.customSelected);
       lineFactory.getOvergroundDeparture($scope.customSelected.code)
			.success(function(stationDetails){
				$scope.stationDetails = stationDetails.departures.all;				
				$log.log(stationDetails.departures.all.length);
				$log.log(stationDetails.departures.all);
			})
  ...
  },
  {
	...
	overgroundController.inject = ['$scope','$log','lineFactory'];

	angular.module('tflFunApp').controller('overgroundController',overgroundController);
})();
